...
void chip_kb_btn_released(CHIP8 chip, uint8_t key) {
	chip->input &= ~1 << key;
}
...
uint8_t *chip_get_vram_ref(CHIP8 chip) {
	return chip->vram;
}
...
bool chip_is_sound_timer_active(CHIP8 chip) {
	return chip->st > 0;
}
	...
	for (j = 0; j < rows; j++) {
		...
		for (i = 0; i < 8; i++) {
			if (pixel & 0x80) {
				...
				/* printf("POS: (%d,%d) V=%d\n", posx, posy, chip->vram[posy * 64 + posx]); */
			}
...
static void execute(CHIP8 chip) {
	chip->exec(chip);
}
